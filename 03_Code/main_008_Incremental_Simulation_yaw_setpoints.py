...
# In addition, it updates the yaw setpoints of the turbines at each incremental step, showcasing how to change control inputs during an incremental simulation.
...
def main():
    ...
    # create a deep copy of the initial control data to use for plotting later
    control_data_initial = oi_inc.settings_ctr.copy()
    ...
    for step in np.arange(0.0, sim_end_time, Delta_t):
        ...
        # Update the yaw setpoints of the turbines 
        # Get state of the simulation at the current step
        current_state = oi_inc.get_state()

        print(f"Current lut at step {step}:\n{current_state.controller.lut}")
        print(f"Current t at step {step}:\n{current_state.controller.t}")
        ...
        print(f"Current orientation at step {step}:\n{orientation_current}")

        # Generate a random sign +-1
        r_sign = np.random.choice([-1, 1])

        # Fill control_data with 4 equal rows of the current orientation
        current_state.controller.lut = np.tile(orientation_current, (4, 1))
        current_state.controller.lut[2][0] += r_sign * 5.0 # Change the yaw of turbine 1 by +-5 degrees randomly
        current_state.controller.lut[3][0] += r_sign * 5.0 # Change the yaw of turbine 2 by +-5 degrees randomly

        current_state.controller.t   = [step, step + Delta_t/2, step + Delta_t/2 +15.0, sim_end_time]
        print(f"New lut at step {step}:\n{current_state.controller.lut}")
        print(f"New t at step {step}:\n{current_state.controller.t}")

        oi_inc.set_state(current_state)
    ...
    # ======================================================= #
    # Plot applied control and measurements compared to initial control 
    # ======================================================= #
    ...
    axs[0].plot(measurements_inc[(measurements_inc['t_idx'] == 0)]['time'],
                 ...
                 color='orange', linestyle='-', label='Power T1')
    axs[0].vlines(np.arange(0, sim_end_time, Delta_t),
                color='gray', linestyle=':', alpha=0.5,
                ymin=0, ymax=1.1*np.max(measurements_inc['Power_PyWake'])/1e6,
    ...
    axs[0].plot(measurements_inc[(measurements_inc['t_idx'] == 1)]['time'],
                 ...
                 color='orange', linestyle='-', label='Power T2')
    axs[0].set_ylim(0, 1.05*np.max(measurements_inc['Power_PyWake'])/1e6)
    axs[0].set_ylabel('Power (MW)')
    axs[0].set_xlim(0, sim_end_time)
    ...
    axs[1].plot(control_inc[(control_inc['t_idx'] == 0)]['time'],
                 ...
                 color='orange', linestyle='-', label='T1 applied yaw')
    axs[1].plot(control_inc[(control_inc['t_idx'] == 1)]['time'],
                 control_inc[(control_inc['t_idx'] == 1)]['yaw'], 
                 color='orange', linestyle='-', label='T2 applied yaw')
    axs[1]. plot(control_data_initial["orientation_t"],
                 [270 - row[0] for row in control_data_initial["orientation_deg"]],
                 color='blue', linestyle='--', label='T1 initial yaw')
    axs[1]. plot(control_data_initial["orientation_t"],
                 [270 - row[1] for row in control_data_initial["orientation_deg"]],
                    color='blue', linestyle='--', label='T2 initial yaw')
