...
class OFFInterface:
    ...
    def init_simulation_by_path(self, path_to_yaml: str):
        ...
        if self.settings_sol["wake_model"].startswith("FLORIS"):
            tmp_yaml_path = self._gen_FLORIS_yaml(self.settings_wke,
                                                  sim_info["wind_farm"],
                                                  sim_info["ambient"],
                                                  path_to_yaml.parent)  # This might not work on Windows Test
    ...
    def increment_sim(self, dt: float, start_time: float = None):
        """
        Increment the simulation by a given time step.

        Parameters
        ----------
        dt: float
            Time step to increment the simulation (in seconds)
        """
        if self.ready_to_run:
            # Update the end time of the simulation to the current time + dt
            if start_time is not None:
                self.off_sim.settings_sim['time start']     = start_time
                
            self.off_sim.settings_sim['time end']  = start_time + dt
    ...
    def get_applied_control(self) -> pd.DataFrame:
        """
        Returns the applied control generated by the simulation
        """
        return self.control_applied
    ...
    def _run_yaml_to_dict(self, sim_info: dict) -> tuple:
        ...
        if 'sector_average' in sim_info:
            settings_wke['sector_average'] = sim_info['sector_average']
